 ...
 * Converted to C# by Sijia Ma, sm2462@cornell.edu
 */
namespace GoodSelectionProcedure.Utilities
{
    public static class Rinott
    {
        /**
	     * Standard Normal CDF
	     *
         ...
         * @param C The point of evaluation
         * @param lngam LNGAM(N) is LN(GAMMA(N/2))
         * @return The PDF of the Chi^2 distribution with N degress of freedom for N &lt;= 50, evaluated at C
         */
        public static double CHIPDF(int N, double C, double[] lngam)
        {
            double FLN2 = ((double)N) / 2.0;
            double TMP = -FLN2 * Math.Log(2d) - lngam[N - 1] + (FLN2 - 1d) * Math.Log(C) - C / 2.0;
            return Math.Exp(TMP);
        }

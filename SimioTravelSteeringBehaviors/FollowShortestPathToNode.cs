...
{
    public class FollowShortestPathToNodeDefinition : ITravelSteeringBehaviorDefinition
    {
        ...
        public string Name
        {
            get { return "Follow Network Path"; }
        }
        ...
        {
            get { return "Steers an entity to follow a specified network path from a starting node to an ending node, staying within the boundaries of a specified path width."; }
        }
        ...
        static readonly Guid MY_ID = new Guid("{6CBBBD77-BA6D-45D1-BCC7-0ED1BBD08AE8}");
        ...
        {
            return new FollowShortestPathToNode(properties, executionContext);
        }
    ...
    class FollowShortestPathToNodeManager
    {
        ...
        const string DATA_KEY = "511c979b-f0e7-44d6-8d28-e5beb409b584";
        ...
        readonly HashSet<FollowShortestPathToNode> _activeSteeringRules = new HashSet<FollowShortestPathToNode>();
        public void AddSteeringRule(FollowShortestPathToNode rule)
        {
        ...
        public void RemoveSteeringRule(FollowShortestPathToNode rule)
        {
        ...
        class UpdateInfo
        {
            public UpdateInfo(FollowShortestPathToNode rule, TravelSteeringMovement movement, bool bFinished, FacilityDirection goalDir)
            {
                RuleInstance = rule;
                Movement = movement;
                Finished = bFinished;
            ...
            public FollowShortestPathToNode RuleInstance { get; }
        ...
        void DoTimeStep()
        {
            ...
            System.Threading.Tasks.Parallel.ForEach(_activeSteeringRules, (rule, _, j) =>
            {
                ...
                var steerInfo = new SteerInfo()
                {
                    ...
                    EntityPsychophysicalRadius = FollowShortestPathToNode.PsychophysicalRadiusOf(entityData),
                    EntityRadius = FollowShortestPathToNode.RadiusOf(entityData),
                ...
                var movement = rule.ComputeSteeringMovement(steerInfo, _timeStep, out bFinished, out goalDir);

                _updateList[j] = new UpdateInfo(rule, movement, bFinished, goalDir);
            });
            ...
            foreach(var item in _updateList)
            {
                ...
                if (item.Finished)
                {
                    var finishedTrace = item.RuleInstance?.FinishedTrace;
                    if (finishedTrace != null)
                        item.RuleInstance.TravelContext.ExecutionInformation.TraceInformation(finishedTrace);
        ...
        private void UpdateCachedEvasionTargets()
        {
            ...
            foreach (var agent in _facilityScope.AgentsInFreespace)
            {
                ...
                var evasionTargetInfo = new EvasionTargetInfo()
                {
                    ...
                    Radius = FollowShortestPathToNode.RadiusOf(agent)
                };
    ...
    class FollowShortestPathToNode : ITravelSteeringBehavior
    {
        ...
        public FollowShortestPathToNode(IPropertyReaders properties, ITravelSteeringBehaviorCreationContext context)
        {
        ...
        /// <summary>
        /// The message to write to the trace when <see cref="ComputeSteeringMovement(SteerInfo, double, out bool, out string, out FacilityDirection)"/> sets bFinished to true
        /// </summary>
        /// <remarks>
        /// This used to be returned as part of <see cref="ComputeSteeringMovement(SteerInfo, double, out bool, out FacilityDirection)"/>, however 
        /// that runs on a different thread than the main run thread, and attempting to get the HierarchalDisplayName from a thread other than the run
        /// thread can cause problems (and now throws a proper cross-thread exception).
        /// </remarks>
        public string FinishedTrace => String.Format("Entity '{0}' finished free space travel movement following the shortest path from '{1}' to '{2}' using network '{3}'.",
                    _context.EntityData.HierarchicalDisplayName,
                    _startingNode.HierarchicalDisplayName,
                    _endingNode.HierarchicalDisplayName,
                    _networkData.HierarchicalDisplayName);

        public TravelSteeringMovement ComputeSteeringMovement(SteerInfo steerInfo, double timeInterval, out bool bFinished, out FacilityDirection goalDir)
        {
            //
            ...
            //
            var distanceFromLastNode = (_endingNode.Location - steerInfo.EntityLocation).ComputeLength();
            if (double.IsInfinity(_agentData.Velocity) || distanceFromLastNode < _lastLinkWidth / 2.0 || distanceFromLastNode < (steerInfo.EntityVelocity * timeInterval))
            {
                //
            ...
            var forceGoal = ComputeGoalForce(steerInfo, timeInterval, out goalDir);
        ...
        static FollowShortestPathToNode()
        {

...
{
    #region SmallestValueFirstRuleDefinition Class

    public class SmallestValueFirstRuleDefinition : ISelectionRuleDefinition
    {
        ...
        public string Name
        {
            get { return "Smallest Value First"; }
        }
        ...
        static readonly Guid MY_ID = new Guid("{5B3F97F6-189C-450f-884D-8CDB80DC6FE2}");
        ...
        public void DefineSchema(IPropertyDefinitions schema)
        {
            ...
            pd.Description = "The expression used to get the value for each candidate. In the expression, use the syntax " +
                "Candidate.[Class].[Attribute] to reference an attribute of the candidate (e.g., Candidate.Entity.Priority).";
            ...
            pd.SetDefaultString(rgpd.PropertyDefinitions, PriorityComparisonRule.SmallestValue.ToString());

            pd = rgpd.PropertyDefinitions.AddExpressionProperty("TieBreakerValueExpression", "Candidate.Entity.Priority");
            pd.DisplayName = "Value Expression";
            pd.Description = "The expression used to get the value for each candidate entity. In the expression, use the syntax " +
        ...
        {
            return new SmallestValueFirstRule(properties);
        }
    ...
    #region SmallestValueFirstRule Class

    public class SmallestValueFirstRule : ISelectionRule, ISelectionRuleInitializable, IComparable<ISelectionRule>
    {
        public SmallestValueFirstRule(IPropertyReaders properties)
        {
        ...
        {
            if (otherRule is SmallestValueFirstRule otherSmallestValueFirstRule)
            {
                return -1 * otherSmallestValueFirstRule.CompareTo(_selectionPriorityComparisonValues);
            }
        ...
        private PriorityComparisonRule[] GetPriorityComparisonRules(IExecutionContext context)
        {
            ...
            priorityComparisonRules[0] = new PriorityComparisonRule(SimioSelectionRules.PriorityComparisonRule.SmallestValue, _valueExpressionProperty);

...
namespace GoodSelectionProcedure
{
    public static class EtaFunc
    {
         ...
         */
        public static double eval(double x, int n1, double alpha1, long k)
        {
            double RHS = 1.0 - Math.Pow(1.0 - alpha1, 1.0 / (k - 1.0));
            double LHS = 0.0;

            Normal normDist = new Normal();
            ChiSquared chisqDist = new ChiSquared(n1 - 1);

            for (int I = 0; I < 32; ++I)
            {
                //try {
                LHS += WEX[I] * 4
                    * (1.0 - normDist.CumulativeDistribution(x * Math.Sqrt(X[I])))
                    * chisqDist.Density(X[I])
                    * (1.0 - chisqDist.CumulativeDistribution(X[I]));
         ...
         */
        private static double evalEta(double x, int n1, double alpha1, long k)
        {
            return eval(x, n1, alpha1, k);
        }
        ...
        public static double find_eta(int n1, double alpha1, long nSys)
        {
            ...
            if (((evalEta(a, n1, alpha1, nSys) >= 0) &&
                    (evalEta(b, n1, alpha1, nSys) >= 0))
                    || ((evalEta(a, n1, alpha1, nSys) < 0) &&
                            (evalEta(b, n1, alpha1, nSys) < 0)))
            {
                return 0.0;
            }
            while (Math.Abs(dx) > e && k < 1000 && evalEta(x, n1, alpha1, nSys) != 0)
            {
                x = ((a + b) / 2.0);
                if ((evalEta(a, n1, alpha1, nSys) * evalEta(x, n1, alpha1, nSys)) < 0)
                {
                    b = x;
                    dx = b - a;
                }
